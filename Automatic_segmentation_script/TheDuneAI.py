...
import Automatic_segmentation_script.Generator_v1 as Generator_v1
...
class ContourPilot:
    def __init__(self, model_path, data_path, output_path='./', verbosity=False, pat_dict=None):
        ...
        self.Patients_gen = Generator_v1.Patient_data_generator(self.Patient_dict, predict=True, batch_size=1,
                                                                image_size=512, shuffle=True,
                                                                use_window=True, window_params=[1600, -600],
    ...
    def segment(self):
        if self.model1 and self.Patient_dict and self.Output_path:
            ...
            for img, _, filename, params in tqdm(self.Patients_gen, desc='Progress'):
                ...
                sitk.WriteImage(generated_img, os.path.join(self.Output_path, filename.split('/')[-2] + '.mha'))
